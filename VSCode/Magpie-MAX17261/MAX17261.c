...
void max17261_write_reg(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data, uint16_t len)
{
	...
	MAX32665_I2C_Write(dev_addr, reg_addr, i2c_data, len);
...
void max17261_read_reg(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data, uint16_t len)
{
	...
	MAX32665_I2C_Read(dev_addr, reg_addr, reg_data,len);
}
...
int max17261_i2c_test(void)
{
	...
	printf("\nThis example finds the addresses of MAX17261 connected to the");
...
void max17261_soft_reset(void)
{
	...
	max17261_write_reg(MAX17261_I2C_ADDR,Soft_Reset_addr,&message, 2);
}
...
void Fuel_gauge_data_collect(unsigned long timer_count_value, char * OutputString)
{
	...
	// Add timer_count to log output line
	sprintf(OutputString, "%lu,", timer_count_value * elapsed_time_multiplier);
	...
	tempdata = (max17261_regs[1]<<8) + max17261_regs[0];
	//printf("VCell register = 0x%04x\n",tempdata);
	sprintf(tempstring, "%lf,", (double)((1.25*tempdata/16)/1000)); // Convert data to volts.
	strcat(OutputString, (const char *)&tempstring);
	...
	tempdata = (max17261_regs[1]<<8) + max17261_regs[0];
	sprintf(tempstring, "%lf,", (double)((1.25*tempdata/16)/1000)); // Convert data to volts following formula in Table 2 of data sheet.
	strcat(OutputString, (const char *)&tempstring);
	...
	sprintf(tempstring, "%f,", (double)stempdata/256);
	strcat(OutputString, (const char *)&tempstring);
	...
	sprintf(tempstring, "%f,", (double)stempdata*(double)156.25/1000);
	strcat(OutputString, (const char *)&tempstring);
	...
	sprintf(tempstring, "%i,", stempdata);
	strcat(OutputString, (const char *)&tempstring);
	...
	sprintf(tempstring, "%f,", (double)stempdata/2);
	strcat(OutputString, (const char *)&tempstring);
	...
	sprintf(tempstring, "%f,", (double)stempdata*(double)0.5);
	strcat(OutputString, (const char *)&tempstring);
	...
	sprintf(tempstring, "%f,", (double)tempdata/256);
	strcat(OutputString, (const char *)&tempstring);
	...
	sprintf(tempstring, "%f,", timerh + timer); // time since POR = timerh plus timer
	strcat(OutputString, (const char *)&tempstring);
	...
	tempdata = (max17261_regs[1]<<8) + max17261_regs[0];
	printf("QH register = %f\n",(double)tempdata * (double)0.5/3600000);
	sprintf(tempstring, "%f,", (double)tempdata * (double)0.5/3600000);
	strcat(OutputString, (const char *)&tempstring);
	...
	sprintf(tempstring, "%f,", (double)tempdata * (double)0.5);
	strcat(OutputString, (const char *)&tempstring);
	...
	{
		printf("max17261 I2C Test = %i\n",tempdata);
		sprintf(tempstring, ",I2C Error %i", tempdata);
		strcat(OutputString, (const char *)&tempstring);
	}
	memset(tempstring,0,20);
	sprintf(tempstring, "\n", tempdata);
