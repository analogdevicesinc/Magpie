...
/*Tiny USB defines  end----------------------------------------------------------------------------------------------------*/

/* Defines for mock audio -----------------------------------------------------------------------------------------------------------*/
...
int16_t audio_buff[DEMO_CONFIG_AUDIO_BUFF_LEN_IN_SAMPS];
...
//--------------------------------------------------------------------+
// Device callbacks for tiny USB audio
//--------------------------------------------------------------------+
...
void audio_task(void)
{
    uint16_t audio_transfer[131072];
    ...
    {
        audio_transfer[cnt] = mock_audio_sine_tick();
        tud_audio_write_support_ff(cnt, audio_transfer[cnt], AUDIO_SAMPLE_RATE / 1000 * CFG_TUD_AUDIO_FUNC_1_N_BYTES_PER_SAMPLE_TX * CFG_TUD_AUDIO_FUNC_1_CHANNEL_PER_FIFO_TX);
